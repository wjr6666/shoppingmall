...
import com.lin.lostandfound.dao.ILostThingsDao;
...
public class LostThingsDaoImpl extends BaseDao implements ILostThingsDao {
	...
	public LostThings findOneRecord(long id, boolean isNext) {
		Session session = super.getSession();
		// 选择下一行（上一贴）
		String hql = "from LostThings lost where lost.status !=2 and lost.id > :id or lost.id=(SELECT MAX(lost.id) from lost) order by lost.id asc"; 
		if(isNext)
			// 选择上一行（下一贴）
			hql = "from LostThings lost where lost.status !=2 and lost.id < :id or lost.id=(SELECT MIN(lost.id) from lost) order by lost.id desc";
