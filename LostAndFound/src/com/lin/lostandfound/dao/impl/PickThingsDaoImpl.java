...
import com.lin.lostandfound.dao.IPickThingsDao;
...
public class PickThingsDaoImpl extends BaseDao implements IPickThingsDao {
	...
	public List<PickThings> findAllByKeywords(String keywords,
		...
		if (keywords != null && keywords.length() > 0)
			hql.append("and pick.thingsName like '%" + keywords + "%' ");
		// 降序
		if (desc)
	...
	public List<PickThings> findAllByThingsType(String type,
		...
		if (type != null && !"".equals(type.trim())
				&& !"all".equals(type.trim())) {
			flag = true;
			hql.append("and pick.thingsType = ? ");
		}
	...
	public PickThings findOneRecord(long id, boolean isNext) {
		Session session = this.getSession();
		// 选择下一行（上一贴）
		String hql = "from PickThings pick where pick.status !=2 and pick.id > :id or pick.id=(SELECT MAX(pick.id) from pick) order by pick.id asc"; 
		if(isNext)
			// 选择上一行（下一贴）
			hql = "from PickThings pick where pick.status !=2 and pick.id < :id or pick.id=(SELECT MIN(pick.id) from pick) order by pick.id desc";
